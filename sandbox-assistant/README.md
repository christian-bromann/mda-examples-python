...
a real workspace: `auth.supabase(project_ref=...)` for identity, plus
...
```text
sandbox-assistant/
  agent.py                 # define_deep_agent + staging middleware
...
Starts the agent (`:2024` via `uv run mda dev`) and UI (`:4900`) together. Open
